...
// --- LOGICA AI STABILE (MODELLO 2026) ---
async function tryGenerateContent(ai: GoogleGenAI, prompt: string, isJson: boolean = false): Promise<GenerateContentResponse> {
    // Usiamo gemini-2.0-flash, lo standard per chi ha il billing attivo
    const modelName = 'gemini-2.0-flash';
    
    try {
        const config: any = { temperature: 0.7 };
        
        // Se chiediamo JSON, lo specifichiamo nel prompt per evitare errori di configurazione
        const finalPrompt = isJson 
            ? `${prompt}. Rispondi ESCLUSIVAMENTE con un oggetto JSON valido. Non includere testo prima o dopo.` 
            : prompt;
        ...
        const response = await withTimeout<GenerateContentResponse>(
            ...
            30000,
            "L'AI non ha risposto entro 30 secondi."
        );
        return response;
    } catch (e: any) {
        console.error("Errore critico durante la chiamata AI:", e.message);
        throw e;
    }
}
            ...
            for (let charIdx = 0; charIdx < currentWord.word.length; charIdx++) {
                if (currentWord.word[charIdx] === charOnGrid) {
                    for (const dir of ['down', 'across']) {
                        ...
                        if (testX >= 0 && testY >= 0 && (dir === 'across' ? testX + currentWord.word.length <= MAX_GRID_SIZE : testY + currentWord.word.length <= MAX_GRID_SIZE)) {
...
export const generateCrossword = async (
  ...
  // USIAMO LA VERSIONE STABILE V1 PER IL BILLING
  ...
  if (extraData.contentType === 'crossword') {
      if (mode === 'manual') {
          ...
          } catch (e) { console.error("AI Fallito, lista vuota"); }
      }
  }
  ...
  if (mode === 'ai' && apiKey) {
      ...
      try {
          ...
          const response = await tryGenerateContent(ai, `Scrivi auguri per ${extraData.recipientName}, tema ${theme}, stile ${tone}`);
...
export const regenerateGreetingOptions = async (
    ...
    try {
        ...
        const response = await tryGenerateContent(ai, `Scrivi 5 auguri per ${recipient}, tema ${theme}, ${inst}`);
        return [response.text || getRandomFallback(theme)];
